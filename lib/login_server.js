...
LoginServer.prototype.startInstance = function( instance_id ) {
	...
	var pub = redis.createClient(conf.redis.uri);
	var sub = redis.createClient(conf.redis.uri, {
		return_buffers: true
	});
...
LoginServer.prototype.startup = function( conf ) {
	...
	var db = this.db = redis.createClient(conf.redis.uri, {no_ready_check: true
	});
	db.on('error', function(err){
		console.log('redis error: ' + err);
	});

	this.pub = redis.createClient(conf.redis.uri, {
		no_ready_check: true
	});
	...
	this.sub = redis.createClient(conf.redis.uri, {
		no_ready_check: true
	});
